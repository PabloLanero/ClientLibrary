...
<template>
    <v-card class="signin-card">
        <v-card-title class="signin-title">{{ $t('login.login') }}</v-card-title>
        <Form @submit="submit">
            <v-card-text class="signin-content">
                ...
                :placeholder="$t('sigin.password')"/>
                <ErrorMessage class="signin-error" name="password"/>
                <span v-if="seLogueo" class="signin-error">{{$t('forms.notExist')}}</span>
            </v-card-text>
            <v-card-actions class="signin-actions">
                <button  class="signin-btn" name="btn" >{{$t('login.btnLogin')}}</button>
            </v-card-actions>
        </Form>
    </v-card>
</template>
